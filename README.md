...
- [`kinetico`](skills/kinetico/SKILL.md) — recorre con la mano un CONJUNTO de
  ...
  o una colección con portadas (`estanteria <carpeta>`: libros, películas y
  series, discos, fotos, con la portada sacada de una imagen con su nombre o del
  propio fichero, y autor, año y sinopsis solo desde un `fichas.json` con fuente)
  como una estantería circular:
  ...
  está en la escena montada, sin salir de la carpeta con la que se abrió, y
  rechaza peticiones de otro origen (`puerta_local.py`). Sin
...
| `kinetico.py` | Convierte un CONJUNTO de cosas — una carpeta del disco (`arbol`), un contrato genérico de nodos (`datos <nodos.json>`, un adaptador por fuente) o una colección con portadas (`estanteria`, ver `estanteria.py`) — en un edificio 3D navegable con la mano o el ratón; su servidor (`kinetico_servidor.py`) expone `entrar`/`abrir` SOLO sobre lo que la escena montada ya declara, sin salir de la carpeta con la que se abrió. | Ninguno — uso manual | Nada en `memory/`: LEE la carpeta o el fichero de nodos que se le pida y no escribe nada en ellos; monta una carpeta autocontenida (o en `--salida DIR`) y la sirve por HTTP SOLO en `127.0.0.1` |

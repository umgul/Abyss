...
- [`kinetico`](skills/kinetico/SKILL.md) — walks through a SET of things that
  ...
  <nodos.json>`, one adapter per source) — as a 3D building; or a collection
  with covers (`estanteria <carpeta>`: books, films and series, albums, photos,
  the cover taken from an image with the same name or from the file itself, and
  author, year and synopsis only from a `fichas.json` with a source) as a
  circular shelf: a fist opens the
  ...
  in the mounted scene, never outside the folder it was opened with, and it
  rejects requests from any other origin (`puerta_local.py`). Without
...
| `kinetico.py` | Turns a SET of things — a folder on disk (`arbol`), a generic node contract (`datos <nodos.json>`, one adapter per source) or a collection with covers (`estanteria`, see `estanteria.py`) — into a 3D building navigable by hand or mouse; its server (`kinetico_servidor.py`) exposes `entrar`/`abrir` ONLY over what the mounted scene already declares, never outside the folder it was opened with. | None — manual use | Nothing in `memory/`: READS the folder or node file it's given and writes nothing into it; mounts a self-contained folder (or under `--salida DIR`) and serves it over HTTP ONLY on `127.0.0.1` |

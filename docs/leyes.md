...
| `fondo.py` | Quita el fondo de una foto en local (tres motores: `sistema` de macOS 14+, `modelo` ONNX de 4.574.861 bytes MEDIDOS con `onnxruntime`, `grabcut` de OpenCV), para que `kinetica.py --quitar-fondo` mida la silueta en vez de estimarla | Ley de procedencia del motor: `auto` prueba en orden y SIEMPRE imprime cuál usó, nunca cambia de motor en silencio; el efecto está MEDIDO con la misma foto en sus dos versiones (78.947 píxeles inventados por relleno con fondo, 55 sin él) | El motor `sistema` está ESCRITO PERO NO PROBADO (el paquete se ha medido en Windows) y se declara así, no como medido; en Windows no hay camino del sistema (el botón «Quitar fondo» de Fotos no expone interfaz pública y la segmentación del Windows App SDK pide NPU); `grabcut` es tosco y lo avisa cada vez, nunca se presenta como el bueno |
| `kinetica.py` | De una foto a una carpeta con `piezas.json` + `piezas/<clave>.png` de sus COMPONENTES REALES (con `--regiones`: polígono a mano + `cv2.grabCut`, solapes por prioridad con las máscaras reales; sin regiones, componentes conexos automáticos, avisado como peor) y un visor three.js servido en `127.0.0.1` que la mano maneja por la cámara | Ley de procedencia: con `--regiones`, QUÉ pieza hay y DÓNDE está lo decide quien mira la foto, en un fichero legible — el borde sí lo mide GrabCut; sin regiones, se avisa que el resultado es automático, sin nombre real y peor, nunca se presenta como reconocimiento. La misma ley, aplicada al ENLACE: el sitio oficial solo puede salir de lo leído o visto EN LA FOTO (`--reconocer`, OCR sobre las piezas; o `--reconocimiento f.json`, firmado por quien la miró y rechazado si no dice qué vio), jamás del `titulo` que escribió una persona — sin reconocimiento no hay enlace, y el visor escribe el motivo | No es reconstrucción 3D ni mide profundidad real; `--rellenar` reconstruye lo tapado con `cv2.inpaint` (inventado, contado en `relleno_px`); sin `abyss/vendor/mp/` (se baja aparte con `instalar.py --manos`, no va en el repo) el visor se sirve sin manos, con aviso en pantalla, nunca con una mano fingida; el «holograma» que abre la palma quieta no es un holograma ni realidad aumentada medida: es el objeto montado sobre el vídeo, anclado a la palma (posición por tres puntos, tamaño por el ancho de palma, giro por su base), con grosor fingido apilando copias y el giro limitado — la pantalla lo dice |
